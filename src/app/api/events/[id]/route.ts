...
export async function GET(request: NextRequest, { params }: RouteContext) {
  ...
  const item = await db.select().from(events).where(eq(events.id, parseInt(id))).limit(1)
...
export async function PUT(request: NextRequest, { params }: RouteContext) {
  ...
  const updated = await db.update(events).set(data).where(eq(events.id, parseInt(id))).returning()
...
export async function DELETE(request: NextRequest, { params }: RouteContext) {
  const { id } = await params
  const deleted = await db.delete(events).where(eq(events.id, parseInt(id))).returning()
